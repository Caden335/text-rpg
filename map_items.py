...
class Band(MapItem):
    ...
    def wander(self, say):
        ...
        """
        if ((self.hostile and abs(self.x) <= 15 and abs(self.y) <= 15 and
             not any(item.x == 0 and item.y == 0 for item in map_items))):
            dist_x = -self.x
            dist_y = -self.y
            ...
            if total_dist <= self.movement:
                ...
                print(f'{self.name} has moved {move_y} {ns}'
                      f'and {move_x} {ew} towards you '
